...
func addProjectPost(w http.ResponseWriter, r *http.Request) {
	...
	technologies := [4]string{reactjs, javascript, golang, nodejs}
...
func projectDetail(w http.ResponseWriter, r *http.Request) {
	...
	err = connection.Conn.QueryRow(context.Background(), "SELECT tb_project.id, name_project, start_date, end_date, duration, description, technologies, image, tb_user.name as author FROM tb_project LEFT JOIN tb_user ON tb_project.author_id = tb_user.id WHERE tb_project.id = $1", id).Scan(
		&ProjectDetail.Id, &ProjectDetail.NameProject, &ProjectDetail.StartDate, &ProjectDetail.EndDate, &ProjectDetail.Duration, &ProjectDetail.Description, &ProjectDetail.Technologies, &ProjectDetail.Image, &ProjectDetail.Author)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		w.Write([]byte("message : " + err.Error()))
		return
